...
util.listForEach = function(list, fn) {
  while(list) {
    fn(list.head)
    list = list.tail
  }
...
util.toFriendlyInlineString = function(x) {
  // TODO better handle multiline functions:
  ...
  if(stringified && stringified[0] === '{' &&
    x.constructor && x.constructor.name !== 'Object')
    stringified = '{ ' + x.constructor.name + stringified.substring(1)
  return stringified
}
...
util.getFunctionComment = function(fn) {
...
util.failInstance = function(obj, extraInfo) {
