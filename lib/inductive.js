...
function createAndSolveSpecification(name, args, factory) {
  var spec
  try {
...
inductive.solve = function(spec) {
  var name = spec.name
    , cachedAst = fileContext.getCachedSolutionAst(name)
...
inductive.specify = function(name) {
  return createAndSolveSpecification(name, arguments, function() {
...
inductive.specifyMatch = function(name) {
  ...
  return createAndSolveSpecification(name, rest, function() {
