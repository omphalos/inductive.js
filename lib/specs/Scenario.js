...
function Scenario(specification, condition) {
  ...
  condition.forEach(function(a) {
    typeUtil.jsObjToType(a, self.specification.recordTypesBySignature)
  })
  self.expectations = []
...
Scenario.prototype.context = function(thisArg) {
  typeUtil.jsObjToType(thisArg, this.specification.recordTypesBySignature)
...
Scenario.prototype.getArgString = function() {
  ...
  var bindStr = !this.thisArg ? '' :
    '.bind(' + util.toFriendlyInlineString(this.thisArg) + ')'
  return bindStr + '(' + conditionStr + ')'
}

function validateSignatureType(sigType, typeFilters, inferredType) {
  typeFilters = typeFilters || {}
  if(inferredType === typePlaceholder) return sigType
  var unwrapped = sigType.unwrapSmallTypes()
  if(!(unwrapped.containsType(inferredType, typeFilters))) {
    // Add special case for empty Arrays:
    throw new Error(
      'Specified type ' + sigType +
      ' but inferred incompatible type ' + inferredType +
      ' using typeFilters ' + typeUtil.typeFiltersToString(typeFilters))
  }
  return sigType
}

Scenario.prototype.inferContextType = function(typeFilters) {
  var result = 'thisArg' in this ? typeUtil.jsObjToType(
    this.thisArg, this.specification.recordTypesBySignature) :
    null
  var sig = this.signature
  if(result && sig && sig.hasSpecified.takesContext)
    return validateSignatureType(
      sig.fnType.contextType, typeFilters, result)
  return result
}

Scenario.prototype.inferArgType = function(argIndex, typeFilters) {
  var result = typeUtil.jsObjToType(
    this.condition[argIndex],
    this.specification.recordTypesBySignature)
  var sig = this.signature
  if(sig && sig.fnType.arg.children.length > argIndex)
    return validateSignatureType(
      sig.fnType.arg.children[argIndex], typeFilters, result)
  return result
}

Scenario.prototype.inferReturnType = function(typeFilters) {
  var self = this
    , recs = self.specification.recordTypesBySignature
    , expectationTypes = util.pluck(this.expectations, 'type')
    , returnType = this.returnType
    , typesToUnite = [returnType].concat(expectationTypes)
    , result = typeUtil.addTypes(typesToUnite)
    , sig = this.signature
  if(sig && sig.hasSpecified.returns)
    return validateSignatureType(sig.fnType.returnType, typeFilters, result)
  return result
}
