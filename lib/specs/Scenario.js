...
Scenario.prototype.validateSignature = function() {
  if(!this.signature) return
  var typeFilters = {}
  for(var a = 0; a < this.condition.length; a++)
    this.inferArgType(a, typeFilters)
  this.inferContextType(typeFilters)
  this.inferReturnType(typeFilters)
}

function validateSignatureType(sigType, typeFilters, inferredType) {
  if(!(sigType.containsType(result, typeFilters || {})))
    throw new Error(
...
Scenario.prototype.inferContextType = function(typeFilters) {
  ...
  if(result && sig && sig.hasSpecified.takesContext)
    return validateSignatureType(sig.fnType.contextType, typeFilters, result)
  return result
}
