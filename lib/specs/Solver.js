...
function refineType(state, expressions, varCounter) {
  ...
  var refinementValue = new RefinementExpression('refinement' + varCounter)
  refinementValue.source = bindExpression
...
var candidateStr = 'arg1 : #element -> String'
if(candidateStr === candidate.head.toString() && node+''==="'element -> String") {
  tap.comment('    why not? node: ' + node)
}
    }
  ...
  tap.comment('available expressions:')
  util.listForEach(expressions, function(expr) {
    tap.comment('  ' + expr)
  })
...
function filterCandidateByType(node, candidate, parentId, typeFilterList) {
  ...
  if(!expressionContainsParentType) {
    if(this.options.verbosity >= verbosityLevels.diagnostic)
      tap.comment(
        '  expression ' + candidate.head +
        ' does not contain parent type ' + node)
    return null
  }

  // We know that the expression is a proper/improper superset
  // of the parent type.
  // This may be because of a typeParameter filter.
  // The filtered expression's type
  // is a proper/improper subset of the parent's type.
  // We can now traverse the AST with this type.
