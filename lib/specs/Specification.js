...
function Specification(name) {
  ...
  this.expressions = []
  this.scenarios = []
  this.argType = typePlaceholder
  this.returnType = typePlaceholder
  this.contextType = typePlaceholder
  ...
  this.options = util.extend({}, fileContext.getFileOptions())
...
Specification.prototype.takesResolved = function(argType) {
  if(this.argType instanceof ArgumentsType)
    this.argType.children.push(argType)
  else this.argType = new ArgumentsType([argType])
}

Specification.prototype.takesContext = function(contextType) {
  this.contextType = contextType
}

Specification.prototype.takes = function(argType) {
  this.takesResolved(
    typeUtil.resolveType(
      argType, this.recordTypesBySignature))
}

Specification.prototype.returnsResolved = function(returnType) {
  this.returnType = returnType
}

Specification.prototype.returns = function(returnType) {
  this.returnsResolved(this.resolveType(returnType))
}
...
Specification.prototype.inferArgLength = function() {
  ...
  var self = this
  var argLength =
    self.argType === typePlaceholder ? null :
    self.argType.getChildTypes().length
...
Specification.prototype.inferArgType = function(index) {
  ...
  var argType = this.argType === typePlaceholder ?
    typePlaceholder :
    this.argType.getChildTypes()[index]

  var call = Function.prototype.call
    , argTypesAtEachIndex = this.scenarios.map(inferArgTypeAtIndex)
    , union = typeUtil.addTypes(argTypesAtEachIndex)

  if(argType !== typePlaceholder) {
    var unwrapped = argType.unwrapSmallTypes()
    if(union !== typePlaceholder &&
      !unwrapped.containsType(union.unwrapSmallTypes(), {})) {
      throw new Error(
        'Specified argument type ' + argType + ' ' +
        'but inferred incompatible type from scenarios ' + union)
    }
    return argType
  }
...
Specification.prototype.inferReturnType = function(inputFilterTypes) {
  ...
  function scenarioFilter(scenario) {
    ...
    for(var a = 0; a < inputFilterTypes.length; a++) {
      var inputTypeAtIndex =
        hasContext && !a ? scenario.inferContextType() :
        hasContext ? scenario.inferArgType(a - 1) :
        scenario.inferArgType(a)
      if(!inputFilterTypes[a].containsType(inputTypeAtIndex))
        return false
    }
    return true
  }
  ...
  var scenarioTypes = filteredScenarios.map(inferScenarioReturnType)
    , isAccumulator = this.recursion === 'accumulator'
    , seedType = isAccumulator ? [this.seedType] : []
    , typesToUnite = scenarioTypes.concat(seedType) // no returnType?
    , union = typeUtil.addTypes(typesToUnite)

  if(this.returnType !== typePlaceholder) {
    var unwrapped = this.returnType.unwrapSmallTypes()
    if(union !== typePlaceholder &&
      !unwrapped.containsType(union.unwrapSmallTypes(), {})) {
      throw new Error(
        'Specified type ' + this.returnType + ' ' +
        'but inferred incompatible type from scenarios ' + union)
    }
...
Specification.prototype.inferContextType = function() { 
  ...
  if(this.contextType !== typePlaceholder) {
    var unwrapped = this.contextType.unwrapSmallTypes()
    if(union !== typePlaceholder &&
      !unwrapped.containsType(union.unwrapSmallTypes(), {})) {
      throw new Error(
        'Specified type ' + this.contextType + ' ' +
        'but infer incompatible type from scenarios ' + union)
    }
...
function addMatchArgExpressions(
  inputTypes, parent, inputIndex, refinedTypes, childIndex, fnReturnType, target) {
  ...
  var matchExpression =  new MatchArgumentExpression(
    inputType.children, inputIndex, childIndex, parent, returnType, fnReturnType)
  ...
  for(var c = 0; c < inputType.children.length; c++) {
    ...
    addMatchArgExpressions.call(
      ...
      inputIndex + 1, childRefinedArgTypes, c, fnReturnType, target)
  }
  //matchExpression.toString = function() { return this.argName }
}
...
Specification.prototype.solve = function(cachedSearchPath) {
  ...
  var fnType = self.inferFunctionType()
  var fnEx = self.isConstructor ?
    fnEx = new ConstructorExpression(self) :
    fnEx = new FunctionExpression(self.name, fnType)

  var recCall = []
  if(self.recursion === 'simple') {
    recCall.push(new RecursiveCallExpression(fnType, self.name, fnEx))
  } else if(self.recursion === 'accumulator') {
    var accFnType = fnType.copy()
    accFnType.isSpecificationType = true
    accFnType.arg.children.push(fnType.returnType)
    recCall.push(new RecursiveCallExpression(accFnType, self.name, fnEx))
    fnEx.seed(self.seedCode)
  }
  ...
  if(self.useArgumentsMatch) {
    ...
    var inputTypes = fnType.getInputTypes()
      , unionInputTypes = util.arrayOfType(inputTypes, UnionType)
      , scenarioTypes = util.pluck(unionInputTypes, 'children')
    addMatchArgExpressions.call(this,
      inputTypes, fnEx, 0, [], 0, fnType.returnType, matchArgExpressions)
    console.log('matchArgExpressions.length', matchArgExpressions.length)
    for(var a = 0; a < matchArgExpressions.length; a++) {
      console.log(matchArgExpressions[a].toString())
    }
    /*target.forEach(function(t) {
      console.log()
      console.log('argName', t.argName)
      console.log('parent ' + t.parent)
      console.log('returnType ' + t.returnType)
    ...
    /*
    var nextMatchParent = fnEx

    fnType.getArgTypes().forEach(function(argType, argIndex) {

      if(!(argType instanceof UnionType)) return

      var matchParent = nextMatchParent
        , argName = 'arg' + argIndex
        , isParentRoot = matchParent == fnEx
      ...
      matchArgExpressions.push(matchExpression)

      nextMatchParent = matchExpression
    })
    */
  }
...
function filterUnusedExpressions(unused, ast) {
  ...
  unused = unused.filter(function(ex) {
    ...
    var typeFilters = {}
      , sameConstructor = ex.constructor === ast.expr.constructor
      , contains = ex.containsType(ast.expr, typeFilters)
      , sameTemplate = ex.template === ast.expr.template
      , used = isValue ? sameTemplate : sameConstructor && contains
      , result = !used
    return result
  })
