...
Specification.prototype.getType = function() {
  return this.solution ? this.solution.type : this.calculateType()
}
...
;['takes', 'returns', 'takesContext'].forEach(function(fn) {
  Specification.prototype[fn] = function() {
...
Specification.prototype.calculateArgLength = function(argTypes) {

  var self = this
    , argLength = null

  argTypes.forEach(function(argType, index) {
    if(argLength !== null && argType.length != argLength)
      throw new Error(
        self.name + ' scenario ' + (index + 1) + ' has the wrong arg count')
    argLength = argType.length
  })

  return argLength || 0
}

Specification.prototype.calculateArgType = function(argTypes) {
  var argLength = this.calculateArgLength(argTypes)
    , calculatedArgTypes = []
  for(var i = 0; i < argLength; i++)
    calculatedArgTypes.push(this.calculateArgTypeAtIndex(i, argTypes))
  return new ArgumentsTupleType(calculatedArgTypes)
}

Specification.prototype.calculateArgTypeAtIndex = function(index, argTypes) {
  var distinct = typeUtil.distinctTypes(argTypes.map(function(st) {
    return st[index]
  }))

  if(distinct.length > 1) {
    tap.comment(
      'Unable to infer type of', this.name, 'at argument ' + index + '.')
    tap.comment('Try specifying a union or generic type using `takes`.')
    throw new Error(
      'Specification ' + this.name +
      ' has multiple types at arg ' + index + ':' +
      ' [' + distinct.join(', ') + ']')
  }
...
Specification.prototype.expandVars = function(variable, vars) {
  ...
  if(variable instanceof Type || variable instanceof TypeParameter) {

    // Ensure types are traversed once:
    var alreadyAdded = vars.filter(function(v) {
      return v instanceof Type &&
        (v.owningSpecification === self) ===
        (variable.owningSpecification === self) &&
        v.equals(variable)
    }).length
    ...
    if(variable instanceof FunctionType &&
      variable.owningSpecification !== self) {
    ...
    if(variable instanceof RecordType) {
      ...
      if(self.wildcardBuildingBlocks.members)
        ...
        self.expandedMemberBuildingBlocks.slice().forEach(function(um) {
          variable.children.forEach(function(prop) {
            ...
            if(um.record && !um.record.equals(variable)) return
      ...
      self.expandedMemberUpdateBuildingBlocks.slice().forEach(function(mu) {
        ...
        if(mu.record && !mu.record.equals(variable)) 
...
Specification.prototype.calculateReturnType = function(returnTypes) {

  var distinct = typeUtil.distinctTypes(returnTypes.filter(util.identity))

  if(distinct.length > 1) {
    tap.comment('Unable to infer return type of ' + this.name)
    tap.comment('Try specifying a union or generic type using `returns`.')
    distinct.forEach(function(type) { tap.comment('  ' + type) })
    throw new Error('Specification ' + this.name +
...
Specification.prototype.calculateContextType = function(contextTypes) { 

  var distinct = typeUtil.distinctTypes(contextTypes.filter(util.identity))

  if(distinct.length > 1) {
    tap.comment('Unable to infer context type of ' + this.name)
    tap.comment('Try specifying a union or generic type using `takesContext`.')
    throw new Error(
      'Specification ' + this.name +
      ' has multiple context types: [' + distinct.join(', ') + ']')
  }
...
Specification.prototype.calculateType = function() {
  var sigs = this.scenarios.map(function(scenario) {
    return scenario.calculateTypeSignature()
  })

  var returnType = this.calculateReturnType(util.pluck(sigs, 'returnType'))
    , argType = this.calculateArgType(util.pluck(sigs, 'argTypes'))
    , contextType = this.calculateContextType(util.pluck(sigs, 'contextType'))
...
Specification.prototype.of = function() {
...
Specification.prototype.createSpecificationCallExpression =
...
Specification.prototype.attemptSolve = function(cachedSearchPath, fnEx) {
  ...
  // Create SpecificationCallExpressions
  var specExpressions = self.specificationBuildingBlocks.
    map(function(spec) {
      return self.createSpecificationCallExpression(
        spec.name, spec.getType(), spec.isConstructor)
    })

  // Create AssignedVariableExpressions
  var assignedVariableExpressions = self.assignmentBuildingBlocks.
    map(function(assn) {
      return new AssignedVariableExpression(
        assn.name, assn.spec.getType().returnType)
    })

  // Create the root expression
  var genericFnType = self.calculateType()
    , fnType = typeUtil.instantiateTypeParameters(genericFnType)

  fnEx = fnEx || (
    self.isConstructor ?
      fnEx = new ConstructorExpression(self, fnType) :
  ...
  fnType.owningSpecification = self
  fnEx.type.owningSpecification = self
  ...
  if(options.verbosity >= verbosityLevels.scope) {
    comment()
    comment(self.name + '.attemptSolve :', fnType.toString())
  ...
  while(!self.solution && (c++ < options.maxAstNodes || !options.maxAstNodes)) {
    ...
    var onTraversal = function(traversal, len, typeFilterList) {
      ...
      // Return it.
      return {
        rawAst: traversalUtil.toAst(traversal),
  ...
  if(self.solution && solutionUnused.length) {
    ...
    comment('WARNING', self.name +
      ': expressions specified but absent from all programs:')
