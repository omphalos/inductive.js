...
Specification.prototype.when = function() {
  var self = this
  return DeferredApiCall.callChildren(arguments, function() {
    return new ScenarioSignature(self)
  })
}
...
Specification.prototype.inferArgType = function(index) {
  ...
  if(argType !== typePlaceholder) {
    var unwrappedArgType = argType.unwrapSmallTypes()
    this.scenarios.forEach(function(s) {
      var scenarioArgType = s.inferArgType(index).unwrapSmallTypes()
      if(scenarioArgType === typePlaceholder) return
      if(!unwrappedArgType.containsType(scenarioArgType, {})) {
        throw new Error(
          'Specified type ' + unwrappedArgType + ' ' +
          'but inferred incompatible type from scenario ' +
            scenarioArgType)
      }
    })
    return argType
...
Specification.prototype.inferReturnType = function(inputFilterTypes) {
  ...
  if(this.returnType !== typePlaceholder) {
    var unwrappedReturnType = this.returnType.unwrapSmallTypes()
    filteredScenarios.forEach(function(s) {
      var scenarioReturnType = s.inferReturnType().unwrapSmallTypes()
      if(scenarioReturnType === typePlaceholder) return
      if(!unwrappedReturnType.containsType(scenarioReturnType, {})) {
        throw new Error(
          'Specified type ' + unwrappedReturnType + ' ' +
          'but inferred incompatible type from scenario ' +
            scenarioReturnType)
      }
    })
