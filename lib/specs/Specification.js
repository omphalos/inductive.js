...
Specification.prototype.inferReturnType = function(argTypes) {

  function scenarioFilter(scenario) {
    if(!argTypes) return true
    var argLength = scenario.condition.length
    for(var a = 0; a < argLength; a++) {
      var argTypeAtIndex = scenario.inferArgType(a)
      if(!argTypes[a].containsType(argTypeAtIndex)) return false
    }
    return true
  }
  ...
  if(argTypes && !filteredScenarios.length)
    throw new Error(
      'Unable to find scenario taking arguments: ' + argTypes.join(', '))
