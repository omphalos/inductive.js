...
Specification.prototype.inferReturnType = function(inputFilterTypes) {
  ...
  function scenarioFilter(scenario) {
    ...
    for(var a = 0; a < inputLength && a < inputFilterTypes.length; a++) {
      var inputTypeAtIndex = hasContext ?
        scenario.inferContextType() :
        scenario.inferArgType(a - 1)
      if(!inputFilterTypes[a].containsType(inputTypeAtIndex)) return false
    }
    return true
  }
