...
Specification.prototype.inferArgType = function(index) {
  ...
  var call = Function.prototype.call
    , argTypes = this.scenarios.map(inferArgTypeAtIndex)
    , union = typeUtil.addTypes(argTypes)
  ...
  if(variable instanceof Type) {
    ...
    if(variable instanceof RecordType) {
      ...
      self.expandedMemberUpdateBuildingBlocks.slice().forEach(function(mu) {
        ...
        if(!allPresent) return git .log('!allPresent')
...
Specification.prototype.inferReturnType = function() {
  ...
  var self = this
    ...
    , scenarioTypes = this.scenarios.map(inferScenarioReturnType)
...
Specification.prototype.inferTypeOfThis = function() { 

  function inferScenarioTypeOfThis(scenario) {
    return scenario.inferTypeOfThis()
  }

  var self = this
    , scenarioTypes = this.scenarios.map(inferScenarioTypeOfThis)
...
Specification.prototype.inferFunctionType = function() {
  var returnType = this.inferReturnType()
    ...
    , contextType = this.inferTypeOfThis()
...
Specification.prototype.solve = function(cachedSearchPath) {
  ...
  if(self.useArgumentsMatch) {
    ...
    fnType.getArgTypes().forEach(function(argType, argIndex) {
      ...
      argMatchExpressions.push(matchExpression)
  ...
  if(self.expandedObjectCreateBuildingBlocks.length) {
    comment('"objectCreate" expression specified for absent records in "' +
      self.name + '":')
    expandedObjectCreateBuildingBlocks.forEach(function(r) {
      comment('  ' + r)
    })
    return null
  }
  ...
  function validateAssignments(assns) {
    assns.forEach(function(a) {
      ...
      if(!(a.name in mockValidations)) {
        comment('Unable to find mock for assignment ' + ie)
        areDependenciesValid  = false
        return
      }
  ...
  while(!self.solution && (c++ < options.maxAstNodes || !options.maxAstNodes)) {
    ...
    var solver = new Solver(c, self.ignoredTypes, options)
...
filterUnusedExpressions = function(unused, ast) {
  var self = this
