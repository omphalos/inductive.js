...
typeUtil.distinctTypes = function(types) {
  ...
  types.forEach(function(type) {
    if(type === typePlaceholder) return
...
typeUtil.addTypes = function(types) {
  if(!types.length) throw new Error('No types to unite')
...
typeUtil.jsObjToType = function(obj, specifiedType, typeFilters, recs) {
  ...
  if(obj instanceof Object) {
    ...
    if(isArgumentsObject) {
      ...
      return nativeTypes.argumentsObjectType.filterType({ a: distinct[0] })
    }
...
typeUtil.resolveType = function(type) {
  ...
  if(type === typePlaceholder) return typePlaceholder
