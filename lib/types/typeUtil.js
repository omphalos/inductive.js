...
typeUtil.distinctTypes = function(types) {
  ...
  types.forEach(function(type) {
    if(!(type instanceof Type) && !(type instanceof TypeParameter)) {
      tap.comment('Unexpected: ' + type)
      throw new Error(
        'typeUtil.addTypes called without a Type or TypeParameter: ' + type)
    }
...
typeUtil.jsObjToType = function(obj, specifiedType, typeFilters, recs) {
  ...
  if(specifiedType) {
    ...
    var specError = specifiedType.validateInstance(obj, typeFilters)
    if(specError.length) {
      tap.comment(
...
typeUtil.resolveType = function(type) {
  ...
  if(type instanceof TypeParameter) return type
...
// Type filteirng helper: `of`
Type.prototype.of = function() {
