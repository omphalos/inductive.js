...
typeUtil.asFunctionOfType = asItemOfType('fn', functionTypings, function() {
  return new FunctionType()
})

typeUtil.asRecordOfType = asItemOfType('record', recordTypings, function() {
...
typeUtil.jsObjToRecordType = function(root, recordTypesBySignature) {
  ...
  util.objectGraphForEach(root, function(obj) {
    // Ignore array and arguments types
    if(obj instanceof Array) return
    if(obj instanceof Object && 'callee' in obj && 'length' in obj &&
      obj.constructor.name === 'Object') return
    ...
    if(recordTypesBySignature[signature]) {
      ...
      recordTypesBySignature[signature] = record
    }
  })
...
typeUtil.jsObjToType = function(obj, recordTypesBySignature) {
  ...
  if(obj instanceof Array) {
    ...
    var result = nativeTypes.arrayType.filterType({ a: elementType })
    return result
  }
  ...
  if(obj instanceof Object) {
    ...
    if(isMap) return typeUtil.jsObjToMapType(obj, recordTypesBySignature)
    var isArgumentsObject =
      'callee' in obj &&
      'length' in obj &&
      obj.constructor.name === 'Object'
    if(isArgumentsObject) {
      if(!obj.length) return nativeTypes.argumentsObjectType
      var argType = [].slice.call(obj, 1).reduce(function(prev, curr) {
        return typeUtil.addTypes([
          prev, typeUtil.jsObjToType(curr, recordTypesBySignature)
        ])
      }, typeUtil.jsObjToType(obj[0], recordTypesBySignature))
      var result = nativeTypes.argumentsObjectType.filterType({ a: argType })
      return result
    }
    return typeUtil.jsObjToRecordType(obj, recordTypesBySignature)
  }
...
typeUtil.resolveType = function(type, recordTypesBySignature) {
  ...
  else if(type === RegExp) nativeResolution = nativeTypes.regExpType
