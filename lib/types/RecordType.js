...
RecordType.prototype.tryFilterType = function(typeFilters, recs) {
  ...
  rec.children = this.children.map(function(child) {
    // TODO remove use of typePlaceholder here
    var childType = child.type.tryFilterType(typeFilters, recs)
    if(!childType) {
      failed = true
      return null
    }
...
RecordType.prototype.takes = function(type, name) {
  ...
  var typeUtil = require('./typeUtil.js')
    , resolved = typeUtil.resolveType(type)
...
RecordType.prototype.getInstanceError = function(obj, typeFilters) {
  ...
  for(var c = 0; c < this.children.length; c++) {
    ...
    var childError = child.type.getInstanceError(obj[child.name], typeFilters)
