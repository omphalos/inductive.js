...
RecordType.prototype.validateInstance = function(obj, typeFilters) {
  if(!obj || !(obj instanceof Object)) return util.failInstance(obj)
  var keys = Object.keys(obj)
  if(!this.ctor.constructsInstance(obj)) return util.failInstance(obj)
  // This check ensures that inheritance works:
  if(obj.constructor !== Object) return []
  if(keys.length !== this.children.length)
    return util.failInstance(obj, 'Expect ' + this.children.length + ' keys')
  for(var c = 0; c < this.children.length; c++) {
    var child = this.children[c]
    if(!(child.name in obj))
      return util.failInstance(obj, 'Missing ' + child.name)
