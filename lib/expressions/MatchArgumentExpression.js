...
function MatchArgumentExpression(
  ...
  this.constrainParent(function(filteredSelf, parent) {
    // ok if: parent is root and match parent
    // ok if: parent is not root and parent is the matchArgExpression
    var result =
      (parent === parent) ||
      (parent instanceof MatchArgumentExpression &&
        parent.argIndex === parent.argIndex)
    return result
  })
}
